...
import exoplasimlegacy.pyburn
import exoplasimlegacy.filesupport
from exoplasimlegacy.filesupport import SUPPORTED
import exoplasimlegacy.randomcontinents
import exoplasimlegacy.makestellarspec
import platform
...
class Model(object):
    ...
    source : str, optional
        ...
        boundary conditions, etc. If not set, will default to exoplasimlegacy/plasim/run/.
    ...
    def __init__(self,resolution="T21",layers=10,ncpus=4,precision=8,debug=False,inityear=0,
        ...
        if not os.path.isfile(sourcedir+"/firstrun"): #This means we haven't run yet, and have some post-install work to do
            recompile=True
            #os.system('spth=$(python%s -c "import exoplasimlegacy as exo; print(exo.__path__)") && echo $spth>sourcepath'%sys.version[0])
    ...
    def cfgpostprocessor(self,ftype="regular",
        ...
        All supported formats can be read by :py:func:`exoplasimlegacy.gcmt.load() <exoplasimlegacy.gcmt.load>` and
        ...
        variables : list or dict, optional
            ...
            to :py:func`pyburn.advancedDataset() <exoplasimlegacy.pyburn.advancedDataset>`. If None, then ``namelist`` must be set.
        mode : str, optional
    ...
    def postprocess(self,inputfile,variables,ftype="regular",log="postprocess.log",
        ...
        variables : str or list or dict or None
            ...
            :py:func`Model.cfgpostprocessor() <exoplasimlegacy.Model.cfgpostprocessor>` will be used. If the
        ...
        crashifbroken : bool, optional 
            True/False. If True, exoplasimlegacy will run .integritycheck() on the file.
        **kwargs : keyword arguments
            ...
            will override any options set via :py:func`Model.cfgpostprocessor() <exoplasimlegacy.Model.cfgpostprocessor>`
    ...
    def integritycheck(self,ncfile): #MUST pass an output archive that contains surface temperature
        ...
        If the file does not exist, exoplasimlegacy will attempt to create it using the postprocessor.
    ...
    **Model Operation**
            ...
            threshold : float, optional 
               Energy balance threshold model should run to, if using :py:func:`runtobalance() <exoplasimlegacy.Model.runtobalance>`.
    ...
    **Radiation**
            ...
            starspec : str, optional
               ...
               2048 wavelengths. Appropriately-formatted files can be created with :py:mod:`makestellarspec.py <exoplasimlegacy.makestellarspec>`.
...
See Also
--------
    :py:func:`modify <exoplasimlegacy.Model.modify>` : Change model configuration after it has been initialized
...
----------
    ...
    def loadconfig(self,configfile):
        ...
        See Also
        --------
        :py:func:`exportcfg <exoplasimlegacy.Model.exportcfg>` : Export model configuration to a text file.
    ...
    def modify(self,**kwargs):
        """Modify any already-configured parameters. All parameters accepted by :py:func:`configure() <exoplasimlegacy.Model.configure>` can be passed as arguments.
        
        See Also
        --------
        :py:func:`configure <exoplasimlegacy.Model.configure>` : Set model parameters and boundary conditions
    ...
    def save(self,filename=None):
        ...
        users, it is recommended that you use the :py:func:`exportcfg <exoplasimlegacy.Model.exportcfg>` function.

        See Also
        --------
        :py:func:`exportcfg <exoplasimlegacy.Model.exportcfg>` : Export model configuration to a portable text file.
...
"""
    ...
    def exportcfg(self,filename=None):
        ...
        See Also
        --------
        :py:func:`loadconfig <exoplasimlegacy.Model.loadconfig>` : Load a saved configuration.
"""
        if not filename:
...
class TLaquaplanet(Model):
    ...
    Identical to :py:class:`Model <exoplasimlegacy.Model>`, except configuration options suitable for
...
class Earthlike(Model):
    ...
    Identical to :py:class:`Model <exoplasimlegacy.Model>`, except configuration options common for
